...
val data = sc.textFile("./data/*.gz", '\n') 

val DTG_GPS = data.map(line => line.split("\\|")).map(arr => (Array(arr(2), arr(3), arr(4)).mkString("_"), List(arr(13).toInt/1000000.0, arr(12).toInt/1000000.0))) 
